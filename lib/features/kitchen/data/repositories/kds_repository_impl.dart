...
class KDSRepositoryImpl implements IKDSRepository {
  ...
  Future<KDSTicket> createTicket({
    ...
    await _db.transaction(() async {
      // 1. Insert Ticket
      await _db.into(_db.kitchenTicketTable).insert(KitchenTicketTableCompanion(
            uuid: Value(uuid),
      ...
      for (var item in items) {
        ...
        await _db.into(_db.kitchenItemTable).insert(KitchenItemTableCompanion(
  ...
  Future<List<KDSTicket>> getActiveTickets({
    ...
    if (includeDone) {
      ...
      query.where((t) => t.status
          .isIn([KDSTicketStatus.fired.name, KDSTicketStatus.prep.name]));
    }
  ...
  Future<void> updateTicketStatus(
    ...
    if (status == KDSTicketStatus.prep) {
      ...
      await (_db.update(_db.kitchenTicketTable)
            ..where((t) => t.uuid.equals(ticketUuid)))
          .write(KitchenTicketTableCompanion(
        status: Value(status.name),
        startedAt: Value(now), // Simplified: always update start time on prep
      ));
    } else if (status == KDSTicketStatus.ready ||
  ...
  Future<KDSExpoSummary> getExpoSummary() async {
    ...
    for (var t in activeTickets) {
      if (t.status == KDSTicketStatus.prep)
        itemsInPrep += t.items.length; // Approximate
      if (t.status == KDSTicketStatus.ready) itemsReady += t.items.length;

      for (var item in t.items) {
        if (item.status != KDSTicketStatus.served &&
            item.status != KDSTicketStatus.voided) {
  ...
  Stream<KDSExpoSummary> watchExpoSummary() {
    ...
    return _db.kitchenTicketTable
        .select()
        .watch()
        .asyncMap((_) => getExpoSummary());
  }
  ...
  Future<List<KDSItem>> _getItemsForTicket(String ticketUuid) async {
    ...
    return rows
        .map((row) => KDSItem(
              ...
              note: row.note,
              course: KDSCourse.values.firstWhere((e) => e.name == row.course,
  ...
  KDSTicket _mapToTicket(KitchenTicketTableData row, List<KDSItem> items) {
    return KDSTicket(
      ...
      items: items,
      status: KDSTicketStatus.values.firstWhere((e) => e.name == row.status,
          orElse: () => KDSTicketStatus.fired),
      firedAt: row.firedAt,
