...
class ReservationTimelinePage extends StatefulWidget {
  ...
  @override
  State<ReservationTimelinePage> createState() =>
      _ReservationTimelinePageState();
}
...
class _ReservationTimelinePageState extends State<ReservationTimelinePage> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      // Start of Day
      final start = DateTime(widget.selectedDate.year,
          widget.selectedDate.month, widget.selectedDate.day, 0, 0);
  ...
  Widget build(BuildContext context) {
    ...
    final tablesToDisplay = _tables.isNotEmpty
        ? _tables
        : List.generate(
            5, (i) => SavvyTable(id: 't$i', name: 'Table ${i + 1}'));
            ...
            children: [
              ...
              Expanded(
                child: SingleChildScrollView(
                  controller:
                      _horizontalScroll, // Sync attempt (basic) - actually triggers error if attached twice.
                  ...
                  child: SizedBox(
                    ...
                    height: _rowHeight *
                        tablesToDisplay.length.toDouble(), // Approx
                    child: Stack(
                      children: [
                        // Background Grid
                        Column(
                          children: tablesToDisplay
                              .map((t) => Container(
                                    height: _rowHeight,
                        ...
                        ..._reservations.map((res) {
                          // Find Table Y
                          final tableIndex = tablesToDisplay
                              .indexWhere((t) => t.id == res.tableUuid);
                          if (tableIndex == -1)
                            return const SizedBox
                                .shrink(); // Unassigned or Deleted table

                          final top = tableIndex * _rowHeight + 5;

                          // Find Time X
                          final startMinutes =
                              (res.reservationTime.hour - _startHour) * 60 +
                                  res.reservationTime.minute;
