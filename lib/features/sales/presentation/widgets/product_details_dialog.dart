...
class ProductDetailsDialog extends StatefulWidget {
  ...
  final Function(Product p, int qty, List<String> mods, String? note)
      onAddToCart;
                        ...
                        children: [
                          ...
                          Wrap(
                            ...
                            children: group.items.map((item) {
                              final isSelected =
                                  _selectedModifiers[group.group.uuid]
                                          ?.contains(item.uuid) ??
                                      false;
                              return FilterChip(
                                label: Text(
                                    '${item.name} (+${item.priceDelta.toStringAsFixed(2)})'),
                                selected: isSelected,
                                onSelected: (selected) {
                                  setState(() {
                                    final current =
                                    ...
                                    if (selected) {
                                      // Check max selection
                                      if ((group.group.maxSelection ?? 0) > 0 &&
                                          current.length >=
                                              (group.group.maxSelection ?? 0)) {
                                        // Show hint?
                                        return;
                                      }
                ...
                children: [
                  ...
                  ElevatedButton.icon(
                    ...
                    onPressed: () {
                      ...
                      final allMods = _selectedModifiers.values
