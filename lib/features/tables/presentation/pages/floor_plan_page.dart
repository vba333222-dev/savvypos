      ...
      child: Scaffold(
        body: ValueListenableBuilder<FloorMode>(
          ...
          builder: (context, mode, child) {
            return _FloorPlanCanvas(
                mode: mode,
                onModeToggle: () {
                  _mode.value = mode == FloorMode.service
                      ? FloorMode.layout
                      : FloorMode.service;
                });
          },
...
class _FloorPlanCanvasState extends State<_FloorPlanCanvas> {
  ...
  void _handleDragUpdate(DragUpdateDetails details, Size tableSize) {
    ...
    if (snappedX > canvasWidth - tableSize.width)
      snappedX = canvasWidth - tableSize.width;
    if (snappedY > canvasHeight - tableSize.height)
      snappedY = canvasHeight - tableSize.height;

    setState(() {
      _ghostRect =
          Rect.fromLTWH(snappedX, snappedY, tableSize.width, tableSize.height);
    });
  }
        ...
        InteractiveViewer(
          ...
          child: DragTarget<String>(
              ...
              onAcceptWithDetails: (details) {
                ...
                if (snappedX < 0) snappedX = 0;
                if (snappedY < 0) snappedY = 0;
                if (snappedX > canvasWidth - 100)
                  snappedX = canvasWidth - 100; // conservative clamp
                if (snappedY > canvasHeight - 100)
                  snappedY = canvasHeight - 100;
                ...
                context
                    .read<TableBloc>()
                    .add(TableEvent.moveTable(tableUuid, normX, normY));
                  ...
                  child: CustomPaint(
                    painter: _DotGridPainter(
                        color: theme.colors.borderDefault.withValues(
                            alpha:
                                widget.mode == FloorMode.layout ? 0.4 : 0.12)),
                        ...
                        return Stack(
                          children: [
                            ...
                            ...state.tables.map((table) {
                              ...
                              final double width =
                                  t.capacity > 4 ? 160.0 : 100.0;
                              final double height =
                                  t.capacity > 4 ? 120.0 : 100.0;
                                ...
                                child: SizedBox(
                                  ...
                                  child: MovableTableNode(
                                    ...
                                    onOrderMerge: (src, dst) {
                                      final targetTable = state.tables
                                          .firstWhere((tbl) => tbl.id == dst);
                                      if (targetTable.isOccupied) {
                                        showDialog(
                                          context: context,
                                          builder: (dialogCtx) => AlertDialog(
                                            title: Text('Gabungkan Tagihan?',
                                                style: TextStyle(
                                                    color: theme
                                                        .colors.textPrimary)),
                                            content: Text(
                                                'Apakah Anda yakin ingin memindahkan tagihan dari Meja $src ke Meja ${targetTable.name}?',
                                                style: TextStyle(
                                                    color: theme
                                              ...
                                              ElevatedButton(
                                                ...
                                                onPressed: () {
                                                  ...
                                                  context.read<TableBloc>().add(
                                                      TableEvent.mergeTables(
                                                          src, dst));
                                                  ScaffoldMessenger.of(context)
                                                      .showSnackBar(const SnackBar(
                                                          content: Text(
                                                              'Menggabungkan Tagihan...')));
                                                },
                                        ...
                                        // Transfer
                                        context.read<TableBloc>().add(
                                            TableEvent.transferTable(src, dst));
                                        ScaffoldMessenger.of(context)
                                            .showSnackBar(SnackBar(
                                                content: Text(
                                                    'Memindah pesanan ke Meja ${targetTable.name}...')));
                                      }
                                    },
            ...
            child: Center(
              child: BlocBuilder<TableBloc, TableState>(
                builder: (context, state) {
                  ...
                  final occupied =
                      state.tables.where((t) => t.isOccupied).length;
          ...
          FilledButton(
            onPressed: () {
              if (controller.text.isNotEmpty) {
                ...
                context
                    .read<TableBloc>()
                    .add(TableEvent.addTable(controller.text, 500, 500));
