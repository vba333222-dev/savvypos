...
class _PaymentSheetState extends State<PaymentSheet> {
  ...
  Future<void> _handleGiftCardPayment() async {
    ...
    final result = await showDialog<Map<String, dynamic>>(
      context: context,
      builder: (_) => GiftCardPaymentDialog(totalDue: widget.totalAmount - _cashAmount), // Allow split? For now just remaining total
    );
  ...
  Future<void> _handleLoyaltyPayment(BuildContext context, LoyaltyMember member, LoyaltyConfig? config) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Use $pointsNeeded points to pay \$${toRedeem.toStringAsFixed(2)}?'),
        actions: [
    ...
    if (confirm == true && mounted) {
       context.read<CartBloc>().add(CartEvent.checkoutProcessed(
         ...
         paymentMethod: 'LOYALTY_POINTS', // Ensure this enum/string is handled in CartBloc/Repo
       ));
       Navigator.pop(context);
    }
  }
              ...
              Row(
                children: [
                  TextButton.icon(
                    ...
                    icon: Icon(Icons.card_giftcard, color: theme.colors.brandPrimary),
                    label: Text("Gift Card", style: TextStyle(color: theme.colors.brandPrimary)),
                  ),
                  // Loyalty Button
                  BlocBuilder<LoyaltyBloc, LoyaltyState>(
                    builder: (context, state) {
                      if (!state.isEnrolled || state.member == null) return const SizedBox.shrink();
                      final points = state.member!.currentPoints;
                      // Simple check: > 100 points
                      if (points < 100) return const SizedBox.shrink();

                      return TextButton.icon(
                        onPressed: () => _handleLoyaltyPayment(context, state.member!, state.config),
                        icon: Icon(Icons.stars, color: Colors.amber.shade700),
                        label: Text("Use Points ($points)", style: TextStyle(color: Colors.amber.shade700)),
                      );
                    },
                  ),
