...
class DeliveryRepositoryImpl implements IDeliveryRepository {
  ...
  Future<List<DeliveryChannelConfig>> getConfigs() async {
    ...
    return rows
        .map((row) => DeliveryChannelConfig(
              id: row.id,
              provider: DeliveryChannel.values.firstWhere(
                  (e) => e.name == row.provider,
                  orElse: () => DeliveryChannel.unknown),
  ...
  Future<void> updateConfig(DeliveryChannelConfig config) async {
    await db
        .into(db.deliveryChannelTable)
        .insertOnConflictUpdate(DeliveryChannelTableCompanion(
          id: Value(config.id),
  ...
  Future<String> simulateIncomingOrder(
      DeliveryChannel provider, Map<String, dynamic> payload) async {
    final channelId =
        provider == DeliveryChannel.gojek ? 'go_food' : 'grab_food';
    final config = await (db.select(db.deliveryChannelTable)
    ...
    await db.transaction(() async {
      ...
      await db.into(db.orderItemTable).insert(OrderItemTableCompanion.insert(
            orderUuid: orderUuid,
            productUuid:
                'mock-burger', // Assuming exists or foreign key constraint might fail if strict.
      ...
      await db
          .into(db.deliveryOrderMetadataTable)
          .insert(DeliveryOrderMetadataTableCompanion.insert(
            orderUuid: orderUuid,
            provider: provider.name,
            externalOrderId:
                payload['id'] ?? 'EXT-${const Uuid().v4().substring(0, 8)}',
  ...
  Future<void> updateDeliveryStatus(String orderUuid, String status,
    ...
    await (db.update(db.deliveryOrderMetadataTable)
          ..where((t) => t.orderUuid.equals(orderUuid)))
        .write(DeliveryOrderMetadataTableCompanion(
