                      ...
                      return DropdownButtonFormField<String>(
                        ...
                        items: products
                            .map((p) => DropdownMenuItem(
                                value: p.uuid, child: Text(p.name)))
                            .toList(),
                        onChanged: (val) {
                          if (val != null) {
                            context
                                .read<ReportBloc>()
                                .add(ReportEvent.loadStockLedger(val));
                          }
                        },
                      );
                    },
              ...
              child: BlocBuilder<ReportBloc, ReportState>(
                builder: (context, state) {
                  return state.maybeWhen(
                    ...
                    stockLoaded: (ledger) {
                      ...
                      return ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final color = item.quantityChange > 0
                              ? Colors.green
                              : Colors.red;
                          ...
                          return ListTile(
                            ...
                            subtitle: Text(
                                '${DateFormat('MM/dd HH:mm').format(item.date)} • Ref: ${item.referenceId}'),
                            trailing: Column(
                    ...
                    orElse: () => const Center(
                        child: Text('Select a product to view ledger')),
                  );
                },
              ),
            ),
