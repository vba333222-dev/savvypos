...
class AdvancedInventoryRepositoryImpl implements IAdvancedInventoryRepository {
  ...
  Future<List<Warehouse>> getWarehouses({bool activeOnly = true}) async {
    ...
    return rows.map((r) => Warehouse(
      ...
      name: r.name,
      code: r.code,
      type: WarehouseType.values.firstWhere((e) => e.name == r.type.toLowerCase(), orElse: () => WarehouseType.warehouse),
      address: r.address,
  ...
  Future<Warehouse?> getWarehouse(String uuid) async {
    final row = await (_db.select(_db.warehouseTable)..where((t) => t.uuid.equals(uuid))).getSingleOrNull();
    if (row == null) return null;
    return Warehouse(
      uuid: row.uuid,
      name: row.name,
      code: row.code,
      type: WarehouseType.values.firstWhere((e) => e.name == row.type.toLowerCase(), orElse: () => WarehouseType.warehouse),
      address: row.address,
  ...
  Future<Warehouse> createWarehouse(Warehouse warehouse) async {
    final uuid = _uuid.v4();
    await _db.into(_db.warehouseTable).insert(
  ...
  Future<void> updateWarehouse(Warehouse warehouse) async {
    await (_db.update(_db.warehouseTable)..where((t) => t.uuid.equals(warehouse.uuid))).write(
      WarehouseTableCompanion(
        name: Value(warehouse.name),
        code: Value(warehouse.code),
        type: Value(warehouse.type.name),
  ...
  Future<void> deleteWarehouse(String uuid) async {
    await (_db.update(_db.warehouseTable)..where((t) => t.uuid.equals(uuid))).write(
      WarehouseTableCompanion(
  ...
  Future<List<StockLevel>> getStockLevels({
    ...
    if (effectiveWarehouseUuid == null) {
      final primary = await (_db.select(_db.warehouseTable)..where((t) => t.isPrimary.equals(true))).getSingleOrNull();
    ...
    final query = _db.select(_db.stockLevelTable).join([
      innerJoin(_db.productTable, _db.productTable.uuid.equalsExp(_db.stockLevelTable.productUuid)),
    ]);

    query.where(_db.stockLevelTable.warehouseUuid.equals(effectiveWarehouseUuid));
    ...
    if (lowStockOnly) {
      query.where(_db.stockLevelTable.reorderPoint.isNotNull() & 
                  _db.stockLevelTable.quantity.isSmallerThan(_db.stockLevelTable.reorderPoint));
    }
  ...
  @override
  Future<StockLevel?> getStockLevel(String productUuid, String warehouseUuid) async {
    final query = _db.select(_db.stockLevelTable).join([
      innerJoin(_db.productTable, _db.productTable.uuid.equalsExp(_db.stockLevelTable.productUuid)),
    ]);
  ...
  @override
  Future<void> updateStockLevel(String productUuid, String warehouseUuid, double newQuantity) async {
    await _db.into(_db.stockLevelTable).insertOnConflictUpdate(
  ...
  Future<StockMovement> recordMovement({
    ...
    return _db.transaction(() async {
      ...
      if (batchUuid != null) {
        ...
        if (batch != null) {
          await updateBatchQuantity(batchUuid, batch.currentQuantity + quantityChange);
        }
      }
      
      // 4. Get Product Name for log
      final product = await (_db.select(_db.productTable)..where((t) => t.uuid.equals(productUuid))).getSingle();
      
      // 5. Insert Movement Log
      final uuid = _uuid.v4();
      await _db.into(_db.stockMovementTable).insert(
  ...
  Future<List<StockMovement>> getMovementHistory({
    ...
    if (productUuid != null) query.where((t) => t.productUuid.equals(productUuid));
    if (warehouseUuid != null) query.where((t) => t.warehouseUuid.equals(warehouseUuid));
    if (movementType != null) query.where((t) => t.movementType.equals(movementType.name));
    if (startDate != null) query.where((t) => t.timestamp.isBiggerOrEqualValue(startDate));
    if (endDate != null) query.where((t) => t.timestamp.isSmallerOrEqualValue(endDate));
    ...
    return rows.map((r) => StockMovement(
      ...
      movementType: MovementType.values.firstWhere((e) => e.name == r.movementType, orElse: () => MovementType.adjustment),
  ...
  Future<List<Batch>> getBatches({
    ...
    if (expiringWithinDays && days != null) {
      ...
      query.where((t) => t.expiryDate.isNotNull() & t.expiryDate.isSmallerOrEqualValue(expiryThreshold));
    }
    
    final rows = await query.get();
    return rows.map((r) => Batch(
      uuid: r.uuid,
      productUuid: r.productUuid,
      warehouseUuid: r.warehouseUuid,
      ...
      status: BatchStatus.values.firstWhere((e) => e.name == r.status, orElse: () => BatchStatus.active),
    )).toList();
  }

  @override
  Future<Batch?> getBatch(String uuid) async {
    final r = await (_db.select(_db.batchTable)..where((t) => t.uuid.equals(uuid))).getSingleOrNull();
  ...
  Future<void> updateBatchQuantity(String uuid, double newQuantity) async {
     await (_db.update(_db.batchTable)..where((t) => t.uuid.equals(uuid))).write(
      BatchTableCompanion(
  ...
  Future<void> updateBatchStatus(String uuid, BatchStatus status) {
     return (_db.update(_db.batchTable)..where((t) => t.uuid.equals(uuid))).write(
      BatchTableCompanion(
  ...
  // ===================================
  @override
  Future<List<StockAlert>> getActiveAlerts({AlertType? type, AlertSeverity? severity, String? warehouseUuid, int limit = 50}) async {
    ...
    final rows = await query.get();
    return rows.map((r) => StockAlert(
      uuid: r.uuid,
      productUuid: r.productUuid,
      productName: r.productName,
      alertType: AlertType.values.firstWhere((e) => e.name == r.alertType),
      severity: AlertSeverity.values.firstWhere((e) => e.name == r.severity),
      message: r.message,
  ...
  Future<void> acknowledgeAlert(String uuid, String acknowledgedBy) async {
    await (_db.update(_db.stockAlertTable)..where((t) => t.uuid.equals(uuid))).write(
  ...
  @override
  Future<StockAlertConfig?> getAlertConfig(String productUuid, String? warehouseUuid) async {
    return null; 
  }
  ...
  Future<List<AdvancedStockTransfer>> getTransfers({
    ...
    if (sourceWarehouseUuid != null) query.where((t) => t.sourceWarehouseUuid.equals(sourceWarehouseUuid));
    if (targetWarehouseUuid != null) query.where((t) => t.targetWarehouseUuid.equals(targetWarehouseUuid));
    if (status != null) query.where((t) => t.status.equals(status.name));
    
    final rows = await query.get();
    return rows.map((r) => AdvancedStockTransfer(
      uuid: r.uuid,
      ...
      status: TransferStatus.values.firstWhere((e) => e.name == r.status),
  ...
  @override
  Future<AdvancedStockTransfer> createTransfer(AdvancedStockTransfer transfer) async {
    final uuid = _uuid.v4();
  ...
  @override
  Future<void> updateTransferStatus(String uuid, TransferStatus status, {String? approvedBy}) async {
    await (_db.update(_db.stockTransferTable)..where((t) => t.uuid.equals(uuid))).write(
      StockTransferTableCompanion(
        status: Value(status.name),
  ...
  @override
  Future<void> shipTransfer(String uuid, List<AdvancedStockTransferItem> items) async {
      await updateTransferStatus(uuid, TransferStatus.inTransit);
  }

  @override
  Future<void> receiveTransfer(String uuid, List<AdvancedStockTransferItem> items, String receivedBy) async {
  ...
  // ===================================
  @override
  Future<InventoryDashboardStats> getDashboardStats({String? warehouseUuid}) async {
  ...
  @override
  Future<List<InventoryInsight>> generateInsights({String? warehouseUuid}) async {
    return [
      InventoryInsight(
        id: '1',
  ...
  @override
  Future<List<VarianceReport>> getVarianceReport({String? warehouseUuid, DateTime? startDate, DateTime? endDate}) async {
    return [];
  }
  ...
  @override
  Future<Map<String, double>> getStockTurnover({String? warehouseUuid, DateTime? startDate, DateTime? endDate}) async {
    return {};
  }
  ...
  // ===================================
  @override
  Future<String> startPhysicalCount({required String warehouseUuid, required String initiatedBy, required String initiatedByName, String? auditType}) async {
    return _uuid.v4();
  }

  @override
  Future<void> recordCount({required String countSessionUuid, required String productUuid, required double countedQty, String? countedBy, String? batchUuid}) async {
  }
