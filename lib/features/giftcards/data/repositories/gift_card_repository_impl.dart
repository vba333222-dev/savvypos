...
class GiftCardRepositoryImpl implements IGiftCardRepository {
  ...
  Future<List<GiftCard>> getGiftCards({
    ...
    if (searchQuery != null && searchQuery.isNotEmpty) {
      query.where((t) => t.cardNumber.contains(searchQuery) | t.customerName.contains(searchQuery));
    }
    if (hasBalance == true) {
  ...
  Future<GiftCard> activateGiftCard({
    ...
    await _db.into(_db.giftCardTable).insert(
  ...
  Future<GiftCardTransaction> redeemGiftCard({
    ...
    final newBalance = card.currentBalance - amount;
    final newStatus = newBalance <= 0 ? GiftCardStatus.used : GiftCardStatus.active;
  ...
  Future<GiftCardTransaction> refundToGiftCard({
    ...
    final newStatus = card.status == GiftCardStatus.used ? GiftCardStatus.active : card.status;
  ...
  Future<GiftCardSummary> getSummary({
    ...
    final transactions = await getAllTransactions(startDate: startDate, endDate: endDate);
    
    final activations = transactions.where((t) => t.type == GiftCardTransactionType.activation);
    final redemptions = transactions.where((t) => t.type == GiftCardTransactionType.redemption);
    
    return GiftCardSummary(
      totalCardsIssued: startDate == null 
        ? cards.length 
        : cards.where((c) => c.createdAt != null && c.createdAt!.isAfter(startDate)).length,
      ...
      totalRedemptions: redemptions.fold<double>(0, (sum, t) => sum + t.amount.abs()),
      outstandingLiability: cards.where((c) => c.status == GiftCardStatus.active)
        .fold<double>(0, (sum, c) => sum + c.currentBalance),
      expiredValue: cards.where((c) => c.status == GiftCardStatus.expired)
        .fold<double>(0, (sum, c) => sum + c.currentBalance),
  ...
  Future<GiftCardTransaction> _recordTransaction({
    ...
    await _db.into(_db.giftCardTransactionTable).insert(
