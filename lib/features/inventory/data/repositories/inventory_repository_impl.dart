...
class InventoryRepositoryImpl implements IInventoryRepository {
  ...
  Future<void> saveSupplier(Supplier supplier) async {
    await _db.into(_db.supplierTable).insertOnConflictUpdate(
  ...
  Future<List<PurchaseOrder>> getPurchaseOrders({PoStatus? status}) async {
    ...
    return rows.map((r) => PurchaseOrder(
      uuid: r.uuid,
  ...
  Future<PurchaseOrder?> getPurchaseOrder(String uuid) async {
    final poRow = await (_db.select(_db.purchaseOrderTable)..where((t) => t.uuid.equals(uuid))).getSingleOrNull();
    if (poRow == null) return null;

    final itemsRows = await (_db.select(_db.purchaseOrderItemTable)..where((t) => t.poUuid.equals(uuid))).get();
    
    final items = <PurchaseOrderItem>[];
    for (final i in itemsRows) {
       final product = i.productUuid != null 
           ? await (_db.select(_db.productTable)..where((t) => t.uuid.equals(i.productUuid!))).getSingleOrNull()
           : null;
       final ingredient = i.ingredientUuid != null
           ? await (_db.select(_db.ingredientTable)..where((t) => t.uuid.equals(i.ingredientUuid!))).getSingleOrNull()
           : null;
  ...
  Future<void> savePurchaseOrder(PurchaseOrder po) async {
    await _db.transaction(() async {
      await _db.into(_db.purchaseOrderTable).insertOnConflictUpdate(
      ...
      await (_db.delete(_db.purchaseOrderItemTable)..where((t) => t.poUuid.equals(po.uuid))).go();
  ...
  @override
  Future<void> receivePurchaseOrder(String poUuid, Map<String, double> receivedQuantities) async {
     await _db.transaction(() async {
        ...
        for (final item in po.items) {
           ...
           if (item.productUuid != null) {
              ...
              updateQuery.where((t) => t.ingredientUuid.equals(item.ingredientUuid!));
           }
           ...
           if (rcv > 0) {
             if (item.productUuid != null) {
               await updateStock(
                ...
                final ing = await (_db.select(_db.ingredientTable)..where((t) => t.uuid.equals(item.ingredientUuid!))).getSingle();
        ...
        final newStatus = allReceived ? PoStatus.completed.name : PoStatus.partiallyReceived.name;
        await (_db.update(_db.purchaseOrderTable)..where((t) => t.uuid.equals(poUuid))).write(
          PurchaseOrderTableCompanion(status: Value(newStatus))
        );
     });
  }
  ...
  Future<void> updateStock(String productUuid, double quantityChange, String reason, {String? referenceId}) async {
    ...
    if (currentStock == null) {
      await _db.into(_db.localStocksTable).insert(
      ...
      await (_db.update(_db.localStocksTable)..where((t) => t.id.equals(currentStock.id))).write(
  ...
  Future<double> getStockQuantity(String productUuid) async {
    const warehouseUuid = 'MAIN_WH';
    final row = await (_db.select(_db.localStocksTable)..where((t) => t.productUuid.equals(productUuid) & t.warehouseUuid.equals(warehouseUuid))).getSingleOrNull();
  ...
  Future<void> startStockCount(StockCount count) async {
     await _db.into(_db.stockCountTable).insert(
  ...
  Future<void> submitStockCount(StockCount count) async {
    await _db.transaction(() async {
      await (_db.update(_db.stockCountTable)..where((t) => t.uuid.equals(count.uuid))).write(
      ...
      for (final item in count.items) {
        await _db.into(_db.stockCountItemTable).insert(
  ...
  // ===================================
  @override
  Future<void> receiveGoods(String poUuid, Map<String, double> quantities) async {
  ...
  // ===================================
  @override
  Future<List<WasteRecord>> getWasteReports({DateTime? from, DateTime? to}) async {
