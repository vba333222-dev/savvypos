...
class CheckoutRepositoryImpl implements ICheckoutRepository {
  ...
  Future<void> updateOrderStatus(String orderUuid, String status) async {
      ...
      .write(OrderTableCompanion(
        status: Value(status),
        paymentStatus: Value(status == 'COMPLETED' ? 'PAID' : 'PARTIAL'), // Simplified logic
      ));
  }
  ...
  PaymentTransaction _mapToEntity(PaymentTransactionTableData data) {
    return PaymentTransaction(
      ...
      method: PaymentMethod.values.firstWhere((e) => e.name == data.method, orElse: () => PaymentMethod.custom),
      amount: data.amount,
      tendered: data.tendered,
      change: data.change,
      status: PaymentStatus.values.firstWhere((e) => e.name == data.status, orElse: () => PaymentStatus.paid),
