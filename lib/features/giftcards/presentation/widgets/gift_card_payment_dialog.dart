...
class _GiftCardPaymentDialogState extends State<GiftCardPaymentDialog> {
  ...
  Future<void> _checkBalance() async {
    ...
    try {
      ...
      final card = await _repo.getGiftCardByNumber(code) ??
          await _repo.getGiftCardByBarcode(code);
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                        labelText: 'Card Number / Barcode',
                        border: OutlineInputBorder()),
                    onSubmitted: (_) => _checkBalance(),
                  ),
                ),
                IconButton(
                    icon: const Icon(Icons.search), onPressed: _checkBalance),
            ...
            if (_card != null && _card!.isUsable) ...[
              ...
              Container(
                ...
                child: Column(
                  children: [
                    Text(
                        'Balance: \$${_card!.currentBalance.toStringAsFixed(2)}',
                        style: const TextStyle(
                            fontSize: 18,
                            fontWeight: FontWeight.bold,
                            color: Colors.green)),
                    if (_card!.customerName != null)
                      Text('Holder: ${_card!.customerName}'),
                  ],
                ),
              ),
              const SizedBox(height: 16),
              TextField(
                ...
                decoration: const InputDecoration(
                    labelText: 'Redeem Amount', border: OutlineInputBorder()),
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: (_card != null && _card!.isUsable && !_isLoading)
              ? _redeem
              : null,
