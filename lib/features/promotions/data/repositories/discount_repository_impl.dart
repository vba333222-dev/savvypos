...
class DiscountRepositoryImpl implements IDiscountRepository {
  ...
  Future<Discount> createDiscount(Discount discount) async {
    ...
    await _db.into(_db.discountTable).insert(
      DiscountTableCompanion(
        ...
        tiersJson: Value(jsonEncode(discount.tiers.map((e) => e.toJson()).toList())),
        applicableProductUuidsJson: Value(jsonEncode(discount.applicableProductUuids)),
        applicableCategoryUuidsJson: Value(jsonEncode(discount.applicableCategoryUuids)),
        applicableCustomerUuidsJson: Value(jsonEncode(discount.applicableCustomerUuids)),
  ...
  Future<void> deleteDiscount(String uuid) async {
    await (_db.delete(_db.discountTable)
      ..where((t) => t.uuid.equals(uuid)))
      .go();
  }
  ...
  Future<DiscountValidationResult> validateDiscountCode({
    ...
    if (discount == null) {
      return const DiscountValidationResult(isValid: false, errorMessage: 'Invalid promo code');
    }
    
    if (!discount.isValid) {
      return const DiscountValidationResult(isValid: false, errorMessage: 'Promo code expired or inactive');
    }
  ...
  bool isValidForDateTime(Discount discount, DateTime dateTime) {
    if (discount.validFrom != null && dateTime.isBefore(discount.validFrom!)) return false;
    if (discount.validUntil != null && dateTime.isAfter(discount.validUntil!)) return false;
  ...
  Future<AppliedDiscount> applyDiscount({
    ...
    await _db.into(_db.appliedDiscountTable).insert(
  ...
  double calculateDiscountAmount({
      ...
      case DiscountType.percentage:
        if (discount.scope == DiscountScope.order) {
          ...
          for (final item in orderItems) {
            if (isApplicableToItem(discount, item.productUuid, item.categoryUuid)) {
              amount += (item.price * item.quantity) * (discount.percentageOff ?? 0) / 100;
            }
          }
        }
        break;
    ...
    // Cap max discount
    if (discount.maxDiscountAmount != null && amount > discount.maxDiscountAmount!) {
  ...
  @override
  Future<List<Discount>> getStackableDiscounts(List<String> discountUuids) async {
  ...
  List<QuickDiscount> getQuickDiscounts() {
    return const [
      QuickDiscount(label: 'Employee', percentage: 50, iconType: IconType.employee),
      QuickDiscount(label: 'Military', percentage: 10, iconType: IconType.special),
      QuickDiscount(label: 'Senior', percentage: 15, iconType: IconType.special),
  ...
  Future<AppliedDiscount> applyQuickDiscount({
    ...
    throw UnimplementedError('Detail implementation requires order repository dependency which is circular. Move calculation to domain service/usecase.');
  }
  ...
  Future<DiscountUsageStats> getUsageStats(String discountUuid) async {
    ...
    final totalDiscounted = applied.fold<double>(0, (sum, a) => sum + a.discountAmount);
  ...
  Discount _mapToDiscount(DiscountTableData row) {
    return Discount(
      ...
      tiers: (jsonDecode(row.tiersJson) as List).map((e) => DiscountTier.fromJson(e)).toList(),
      applicableProductUuids: List<String>.from(jsonDecode(row.applicableProductUuidsJson)),
      applicableCategoryUuids: List<String>.from(jsonDecode(row.applicableCategoryUuidsJson)),
      applicableCustomerUuids: List<String>.from(jsonDecode(row.applicableCustomerUuidsJson)),
