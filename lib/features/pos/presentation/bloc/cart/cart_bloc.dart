...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  CartBloc(this.db, this._sound, this._syncWorker, this._socketService,
      this._tenantRepo)
  ...
  Future<void> _onScanItem(ScanItem event, Emitter<CartState> emit) async {
    try {
      ...
      final productRow = await (db.select(db.productTable)
            ..where((t) =>
                t.sku.equals(event.barcode) | t.barcode.equals(event.barcode)))
          .getSingleOrNull();
  ...
  void _onAddProduct(AddProduct event, Emitter<CartState> emit) {
    ...
    if (existingIndex != -1) {
      ...
      // Recalc Item Total
      final unitPrice = event.product.price +
          event.modifiers.fold(0.0, (sum, m) => sum + m.priceDelta);
      final newTotal = unitPrice * newQuantity;

      updatedItems = List.from(state.items);
      updatedItems[existingIndex] =
          existingItem.copyWith(quantity: newQuantity, total: newTotal);
    } else {
      // Add New
      final unitPrice = event.product.price +
          event.modifiers.fold(0.0, (sum, m) => sum + m.priceDelta);
    ...
    final updatedItems = state.items.map((item) {
      if (item.uuid == event.itemUuid) {
        ...
        final unitPrice = item.product.price +
            item.modifiers.fold(0.0, (sum, m) => sum + m.priceDelta);
  ...
  void _onRemoveFromCart(RemoveFromCart event, Emitter<CartState> emit) {
    final updatedItems =
        state.items.where((item) => item.uuid != event.itemUuid).toList();
    emit(_calculateTotals(updatedItems));
  }
  ...
  Future<void> _onCheckoutProcessed(
      CheckoutProcessed event, Emitter<CartState> emit) async {
    if (state.items.isEmpty) return;
      ...
      await db.transaction(() async {
        ...
        for (final item in state.items) {
          ...
          if (item.product.isComposite) {
            // Deduct Ingredients
            final recipeItems = await (db.select(db.recipeTable)
                  ..where((r) => r.productUuid.equals(item.product.uuid)))
                .get();
            for (final r in recipeItems) {
              final qtyToDeduct = r.quantityRequired * item.quantity;

              final ingredient = await (db.select(db.ingredientTable)
                    ..where((i) => i.uuid.equals(r.ingredientUuid)))
                  .getSingleOrNull();
              if (ingredient != null) {
                await (db.update(db.ingredientTable)
                      ..where((i) => i.uuid.equals(ingredient.uuid)))
                    .write(IngredientTableCompanion(
                        currentStock:
                            Value(ingredient.currentStock - qtyToDeduct)));
              }
            }
          } else {
        ...
        final payload = {
          ...
          'items': state.items
              .map((e) =>
                  {'productUuid': e.product.uuid, 'quantity': e.quantity})
              .toList(),
          'subtotal': state.subtotal,
  ...
  void _onAddPromoCode(AddPromoCode event, Emitter<CartState> emit) {
    ...
    if (foundPromo != null) {
      if (foundPromo.isValidNow() &&
          foundPromo.meetsMinPurchase(state.subtotal)) {
        ...
        if (!currentPromos.any((p) => p.code == foundPromo!.code)) {
          ...
          emit(_calculateTotals(
              state.items, state.copyWith(activePromotions: currentPromos)));
        } else {
          emit(state.copyWith(error: 'Promotion already applied'));
        }
      } else {
        emit(
            state.copyWith(error: 'Conditions not met for ${foundPromo.name}'));
      }
    } else {
  ...
  Future<void> _onParkOrder(ParkOrder event, Emitter<CartState> emit) async {
    ...
    try {
      await db.transaction(() async {
        ...
        final orderNumber =
            state.lastOrderNumber ?? 'ORD-${now.millisecondsSinceEpoch}';

        // 1. Upsert Order (Status = OPEN)
        await db
            .into(db.orderTable)
            .insertOnConflictUpdate(OrderTableCompanion.insert(
              uuid: orderUuid,
              orderNumber: orderNumber,
              tenantId: const Value('default-tenant'),
        ...
        await (db.delete(db.orderItemTable)
        ...
        await (db.update(db.restaurantTable)
              ..where((t) => t.uuid.equals(event.tableUuid)))
            .write(
      ...
      emit(CartState.initial()
          .copyWith(isSuccess: true, lastOrderNumber: 'Parked'));
    } catch (e) {
  ...
  Future<void> _onRetrieveOrder(
      RetrieveOrder event, Emitter<CartState> emit) async {
    emit(state.copyWith(isLoading: true));
    try {
      // 1. Fetch Order
      final order = await (db.select(db.orderTable)
            ..where((t) => t.uuid.equals(event.orderUuid)))
          .getSingle();
  ...
  Future<void> _onCheckoutSplit(
      CheckoutSplit event, Emitter<CartState> emit) async {
    ...
    final splitItems =
        state.items.where((i) => event.itemUuids.contains(i.uuid)).toList();
    ...
    try {
      await db.transaction(() async {
        ...
        final total = subtotal +
            tax; // ignoring discount logic for split for simplicity MVP
        ...
        for (var splitItem in splitItems) {
          ...
          final parentItem = await (db.select(db.orderItemTable)
                ..where((t) =>
                    t.orderUuid.equals(state.activeOrderUuid!) &
                    t.productUuid.equals(splitItem.product.uuid)))
              .getSingle();

          await (db.update(db.orderItemTable)
                ..where((t) => t.id.equals(parentItem.id)))
              .write(
        ...
        // Fetch all items
        final allItems = await (db.select(db.orderItemTable)
              ..where((t) => t.orderUuid.equals(state.activeOrderUuid!)))
            .get();
        bool allPaid = true;
        ...
        if (allPaid) {
          ...
          await (db.update(db.orderTable)
                ..where((t) => t.uuid.equals(state.activeOrderUuid!)))
              .write(
          ...
          if (state.activeTableUuid != null) {
            await (db.update(db.restaurantTable)
                  ..where((t) => t.uuid.equals(state.activeTableUuid!)))
                .write(
          ...
          emit(CartState.initial().copyWith(
              isSuccess: true, lastOrderNumber: childOrderNumber)); // Done
        } else {
          ...
          emit(state.copyWith(
              isLoading: false,
              isSuccess: true,
              lastOrderNumber: childOrderNumber));
  ...
  CartState _calculateTotals(List<CartItem> items, [CartState? currentState]) {
    ...
    final promoResult =
        _calculatePromotions(calculatedItems, s.activePromotions);
    ...
    // Cap manual discount
    if (manualDiscountVal > discountedSubtotal)
      manualDiscountVal = discountedSubtotal;
  ...
  ({List<CartItem> items}) _calculatePromotions(
      List<CartItem> items, List<Promotion> promotions) {
    ...
    for (var promo in promotions) {
      ...
      tempItems = promo.map(
        ...
        fixedAmount: (p) =>
            tempItems, // Fixed Amount usually Cart Level. Skipping item distribution for MVP.
  ...
  List<CartItem> _applyPercentage(
      List<CartItem> items, PercentageDiscount promo) {
