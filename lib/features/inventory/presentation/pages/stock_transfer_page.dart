...
class _StockTransferPageState extends State<StockTransferPage> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => GetIt.I<
          InventoryManagementBloc>(), // In real app, might pass existing bloc or create new
      child: BlocListener<InventoryManagementBloc, InventoryManagementState>(
        ...
        child: Builder(// Builder to get context with Bloc
            builder: (context) {
          final isLoading = context.select((InventoryManagementBloc bloc) =>
              bloc.state.maybeWhen(loading: () => true, orElse: () => false));
              ...
              children: [
                    ...
                    children: [
                      ...
                      FilledButton.icon(
                        onPressed: () {
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                              content: Text(
                                  'Searching for: ${_barcodeController.text}')));
                    ...
                    children: [
                      Row(
                        children: [
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Icon(Icons.warehouse,
                                      size: 80,
                                      color: context.savvy.colors.textDisabled),
                                  const SizedBox(height: 16),
                                  const SavvyText.body(
                                      'Product: Espresso Beans'),
                                  const SavvyText.body(
                                      'Current Stock: 150'), // Mock
                                ],
                              ),
                            ),
                          ),
                          ...
                          // DIVIDER
                          VerticalDivider(
                              width: 1, color: context.savvy.colors.border),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      color: context.savvy.colors.bgSurface,
                      border: Border(
                          top: BorderSide(color: context.savvy.colors.border))),
                  child: Center(
                      ...
                      action: (controller) async {
                        ...
                        if (context.mounted) {
                          context.read<InventoryManagementBloc>().add(
                              InventoryManagementEvent.stockTransferRequested(
