  ...
  Widget build(BuildContext context) {
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                image: widget.product.imageUrl != null 
                  ? DecorationImage(image: FileImage(File(widget.product.imageUrl!)), fit: BoxFit.cover)
                  : null
              ),
               child: widget.product.imageUrl == null ? Icon(Icons.inventory_2, color: theme.colors.textMuted) : null,
            ),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('SKU: ${widget.product.sku ?? '-'}', style: TextStyle(color: theme.colors.textSecondary, fontSize: 12)),
              ],
            ),
          ),
             ...
             Container(
               ...
               child: AnimatedSwitcher(
                 ...
                 transitionBuilder: (child, animation) {
                   ...
                   final inOffset = (widget.currentStock > _prevStock) ? const Offset(0, 1) : const Offset(0, -1);
                 ...
                 child: Text(
                   ...
                   style: TextStyle(
                     ...
                     color: isLowStock ? theme.colors.stateError : theme.colors.stateSuccess,
                     fontSize: 16
                   ),
                 ),
               ),
    ...
    return Dismissible(
      ...
      background: Container(
        ...
        child: Row(
          children: [
            _StockActionButton(icon: Icons.remove, onTap: () => widget.onStockUpdate(-1)),
            const SizedBox(width: 24),
            _StockActionButton(icon: Icons.add, onTap: () => widget.onStockUpdate(1), isMajor: true),
          ],
        ),
      ),
...
class _StockActionButton extends StatelessWidget {
  ...
  const _StockActionButton({required this.icon, required this.onTap, this.isMajor = false});

  @override
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        if (isMajor) HapticFeedback.heavyImpact(); else HapticFeedback.mediumImpact();
        onTap();
      },
      child: Container(
        width: 56, height: 56,
