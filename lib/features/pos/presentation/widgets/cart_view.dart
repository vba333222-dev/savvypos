        ...
        children: [
          ...
          BlocBuilder<CartBloc, CartState>(
            builder: (context, state) {
               return _CartFooter(
                 state: state,
                 onChargePressed: (pressed) => setState(() => _isChargePressed = pressed),
                 onChargeTap: () {
  ...
  void _updateList(List<CartItem> newItems) {
    ...
    for (int i = 0; i < newItems.length; i++) {
        ...
        if (!oldIds.contains(item.uuid)) {
           ...
           // Update
           final existingIndex = _displayedItems.indexWhere((e) => e.uuid == item.uuid);
           if (existingIndex != -1) {
...
class _DismissBackground extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Icon(Icons.delete_forever, color: theme.colors.textInverse, size: 32)
