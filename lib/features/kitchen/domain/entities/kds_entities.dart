...
class KDSTicket with _$KDSTicket {
  ...
  factory KDSTicket.fromJson(Map<String, dynamic> json) => _$KDSTicketFromJson(json);
  ...
  KDSAlertLevel get alertLevel {
    ...
    if (now.add(const Duration(minutes: 5)).isAfter(targetTime!)) return KDSAlertLevel.warning;
    return KDSAlertLevel.normal;
  }
  
  /// Get ticket background color based on status/alert
  Color get statusColor {
    if (status == KDSTicketStatus.ready) return const Color(0xFF4CAF50); // Green
    if (status == KDSTicketStatus.voided) return const Color(0xFF9E9E9E); // Grey
...
class KDSItem with _$KDSItem {
  ...
  factory KDSItem.fromJson(Map<String, dynamic> json) => _$KDSItemFromJson(json);
}
...
class KDSExpoSummary with _$KDSExpoSummary {
  ...
  factory KDSExpoSummary.fromJson(Map<String, dynamic> json) => _$KDSExpoSummaryFromJson(json);
}
