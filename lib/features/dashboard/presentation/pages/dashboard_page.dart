...
class _DashboardView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: SavvyText('Dashboard', style: SavvyTextStyle.h3, color: theme.colors.textPrimary),
        backgroundColor: theme.colors.bgSecondary,
        elevation: 0,
        actions: [
          IconButton(
            icon: Icon(Icons.refresh, color: theme.colors.textPrimary),
            onPressed: () => context.read<DashboardBloc>().add(const DashboardEvent.refresh()),
          ),
        ],
      ),
                    ...
                    LayoutBuilder(builder: (context, constraints) {
                      ...
                      return Wrap(
                        ...
                        children: [
                          _StatCard(
                            label: 'Total Sales',
                            value: NumberFormat.simpleCurrency().format(stats.totalSales),
                          ...
                          _StatCard(
                            label: 'Avg Basket',
                            value: NumberFormat.simpleCurrency().format(stats.avgBasketSize),
      ...
      child: Row(
        children: [
          ...
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              SavvyText(label, style: SavvyTextStyle.bodySm, color: theme.colors.textSecondary),
              SavvyText(value, style: SavvyTextStyle.h3, color: theme.colors.textPrimary),
            ],
          ),
        ],
      ),
            ...
            child: LineChart(
              LineChartData(
                gridData: FlGridData(show: true, drawVerticalLine: false, getDrawingHorizontalLine: (value) {
                  return FlLine(color: theme.colors.borderDefault, strokeWidth: 1);
                }),
                titlesData: FlTitlesData(
                  ...
                  bottomTitles: AxisTitles(
                     sideTitles: SideTitles(showTitles: true, interval: 4, getTitlesWidget: (value, meta) {
                ...
                lineBarsData: [
                  LineChartBarData(
                    ...
                    belowBarData: BarAreaData(
                      show: true,
                       color: theme.colors.brandPrimary.withOpacity(0.1),
                    ),
                  ),
                ],
        ...
        children: [
          ...
          if (products.isEmpty)
            Padding(
              ...
              child: Center(child: Text("No items sold yet", style: TextStyle(color: theme.colors.textMuted))),
            )
          else
                ...
                child: Row(
                  children: [
                    Container(
                       ...
                       decoration: BoxDecoration(
                         color: index < 3 ? theme.colors.brandSecondary : theme.colors.bgSecondary,
                         shape: BoxShape.circle,
                       ),
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(p.productName, style: TextStyle(color: theme.colors.textPrimary, fontWeight: FontWeight.w600)),
                          Text('${p.quantity} sold', style: TextStyle(color: theme.colors.textSecondary, fontSize: 12)),
                        ],
                      ),
                    ),
                    Text(
