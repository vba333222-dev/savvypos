...
class ProductRepositoryImpl implements IProductRepository {
  ...
  Stream<List<Product>> watchAllProducts() async* {
    ...
    yield* (db.select(db.productTable)
  ...
  Stream<List<ProductStock>> watchInventory(String warehouseId) async* {
    ...
    yield* query.watch().map((rows) {
      return rows.map((row) {
        ...
        return ProductStock(
            product: product, quantity: stockRow?.quantity ?? 0.0);
      }).toList();
    });
  }
  ...
  Future<Product?> getProductByUuid(String uuid) async {
    final row = await (db.select(db.productTable)
      ...
      final query = db.select(db.modifierGroupTable).join([
        innerJoin(
            ...
            db.productModifierLinkTable.modifierGroupUuid
                .equalsExp(db.modifierGroupTable.uuid))
      ])
  ...
  Future<Either<Failure, List<ModifierGroup>>> getAllModifierGroups() async {
    try {
      ...
      for (var row in rows) {
        // Fetch items
        final itemRows = await (db.select(db.modifierItemTable)
              ..where((t) => t.groupUuid.equals(row.uuid)))
            .get();
        final items = itemRows
            .map((i) => ModifierItem(
                uuid: i.uuid, name: i.name, priceDelta: i.priceDelta))
            .toList();
  ...
  Future<Either<Failure, void>> saveModifierGroup(ModifierGroup group) async {
    try {
      // 1. Save Group
      await db
          .into(db.modifierGroupTable)
          .insertOnConflictUpdate(ModifierGroupTableCompanion(
      ...
      // 2. Save Items
      await (db.delete(db.modifierItemTable)
            ..where((t) => t.groupUuid.equals(group.uuid)))
          .go();

      for (var item in group.items) {
        await db
            .into(db.modifierItemTable)
            .insert(ModifierItemTableCompanion.insert(
              uuid: item.uuid,
  ...
  Future<Either<Failure, void>> deleteModifierGroup(String uuid) async {
    try {
      await (db.delete(db.modifierGroupTable)
  ...
  Future<Either<Failure, void>> updateProductModifiers(
      String productId, List<String> modifierIds) async {
    try {
      await (db.delete(db.productModifierLinkTable)
            ..where((t) => t.productUuid.equals(productId)))
          .go();
      for (var groupUuid in modifierIds) {
        await db
            .into(db.productModifierLinkTable)
            .insert(ProductModifierLinkTableCompanion.insert(
  ...
  Future<Either<Failure, void>> saveIngredient(Ingredient ingredient) async {
    try {
      await db
          ...
          .insertOnConflictUpdate(IngredientTableCompanion(
  ...
  Future<Either<Failure, void>> deleteIngredient(String uuid) async {
    try {
      await (db.delete(db.ingredientTable)..where((t) => t.uuid.equals(uuid)))
          .go();
      return const Right(null);
    } catch (e) {
      return Left(DatabaseFailure(e.toString()));
  ...
  Future<Either<Failure, Recipe?>> getRecipeForProduct(String productId) async {
    try {
      final query = db.select(db.recipeTable).join([
        innerJoin(db.ingredientTable,
            db.ingredientTable.uuid.equalsExp(db.recipeTable.ingredientUuid))
      ])
      ...
      for (var row in rows) {
        ...
        items.add(RecipeItem(
            ingredient: ingredient, quantity: recipeRow.quantityRequired));
      }
      return Right(Recipe(
          productUuid: productId,
          items: items)); // Assuming Recipe entity structure
    } catch (e) {
      return Left(DatabaseFailure(e.toString()));
    }
  }
  ...
  Future<Either<Failure, void>> updateRecipe(Recipe recipe) async {
    try {
      await (db.delete(db.recipeTable)
