...
class EmployeeRepositoryImpl implements IEmployeeRepository {
  ...
  Future<Employee> createEmployee(Employee employee) async {
    ...
    await _db.into(_db.employeeTable).insert(
    ...
    await _db.into(_db.employeeExtendedTable).insert(
  ...
  Future<TimeEntry> clockIn({
    ...
    await _db.into(_db.timeEntryTable).insert(
  ...
  Future<void> endBreak(String timeEntryUuid) async {
    ...
    if (entry.breakStart != null) {
      final breakMins = DateTime.now().difference(entry.breakStart!).inMinutes.toDouble();
  ...
  Future<TimeEntry?> getCurrentTimeEntry(String employeeUuid) async {
    final row = await (_db.select(_db.timeEntryTable)
      ..where((t) => t.employeeUuid.equals(employeeUuid) & t.clockOut.isNull())
  ...
  Future<List<ScheduledShift>> getScheduledShifts({
    ...
    query.orderBy([(t) => OrderingTerm.asc(t.date), (t) => OrderingTerm.asc(t.startTime)]);
  ...
  Future<ScheduledShift> createShift(ScheduledShift shift) async {
    ...
    await _db.into(_db.scheduledShiftTable).insert(
  ...
  Future<void> publishSchedule(DateTime weekStart) async {
    ...
    await (_db.update(_db.scheduledShiftTable)
      ..where((t) => t.date.isBiggerOrEqualValue(weekStart) & t.date.isSmallerThanValue(weekEnd)))
  ...
  Future<List<LaborSummary>> getLaborSummary({
    ...
    for (final emp in employees) {
      ...
      final laborCost = (totalRegular * emp.hourlyRate) + (totalOvertime * emp.hourlyRate * 1.5);
  ...
  Future<double> getTotalLaborCost({
    ...
    final summaries = await getLaborSummary(startDate: startDate, endDate: endDate);
  ...
  Future<List<Employee>> getOvertimeAlerts({
    ...
    final weekEnd = weekStart.add(const Duration(days: 7));
    final summaries = await getLaborSummary(startDate: weekStart, endDate: weekEnd);
    final employees = await getEmployees();
    
    final List<Employee> alerts = [];
    for (final summary in summaries) {
      if (summary.totalHours >= thresholdHours * 0.9) { // 90% of threshold
  ...
  Employee _mapToEmployee(EmployeeTableData row, EmployeeExtendedTableData? extended) {
    return Employee(
      uuid: row.uuid,
      name: row.name,
      pin: row.pin,
      ...
      status: extended != null 
        ? EmploymentStatus.values.firstWhere(
            ...
            orElse: () => row.isActive ? EmploymentStatus.active : EmploymentStatus.inactive,
          )
        : (row.isActive ? EmploymentStatus.active : EmploymentStatus.inactive),
      hourlyRate: extended?.hourlyRate ?? 0,
      salary: extended?.salary ?? 0,
      payType: extended != null 
        ? PayType.values.firstWhere((e) => e.name == extended.payType, orElse: () => PayType.hourly)
        : PayType.hourly,
