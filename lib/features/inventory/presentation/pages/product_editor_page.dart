  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<MenuManagementBloc, MenuManagementState>(
      listener: (context, state) {
        ...
        if (widget.product != null && state.editingProductModifierIds.isNotEmpty && _selectedModifierGroups.isEmpty) {
           setState(() {
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          initialValue: _selectedCategory ?? 'main', // Default or handle null
                          decoration: const InputDecoration(labelText: 'Category', border: OutlineInputBorder()),
                          items: const [
                            DropdownMenuItem(value: 'main', child: Text('Main Menu')),
                      ...
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(labelText: 'Printer Routing', border: OutlineInputBorder()),
                          items: const [
                  ...
                  Row(
                    ...
                    children: [
                      Text('Modifier Groups', style: Theme.of(context).textTheme.titleLarge),
                      TextButton(
                        onPressed: () {
                           // Navigate to Manager?
                           ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Go to "Modifier Groups" page to create new ones.')));
                        },
                        child: const Text('Manage Groups'),
                      )
                    ],
                  ),
                  const Text('Select which modifier options are available for this product.'),
                  const SizedBox(height: 16),
                  
                  if (state.allModifierGroups.isEmpty)
                    const Padding(
                      padding: EdgeInsets.all(16.0),
                      child: Text('No modifier groups available. Create some first!'),
                    )
                  else
                    ...state.allModifierGroups.map((group) {
                      final isSelected = _selectedModifierGroups.contains(group.uuid);
