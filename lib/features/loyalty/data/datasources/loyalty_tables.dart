...
class LoyaltyProgramTable extends Table {
  ...
  // Config
  TextColumn get name => text().withDefault(const Constant('Default Loyalty'))();
  RealColumn get pointsPerCurrency => real().withDefault(const Constant(1.0))(); // e.g. 1 point per $1
  RealColumn get exchangeRate => real().withDefault(const Constant(0.01))(); // e.g. 100 points = $1
  
  // Tiers (JSON or separate table? Simple JSON for MVP config)
  TextColumn get tiersJson => text().nullable()(); // List of tiers: Bronze, Silver, Gold
...
class LoyaltyRewardTable extends Table {
  ...
  TextColumn get applicableProductUuid => text().nullable()(); // If free item/specific discount
