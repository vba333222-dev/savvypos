...
class CashManagementEvent with _$CashManagementEvent {
  ...
  const factory CashManagementEvent.cashDrop(double amount, String reason) = _CashDrop;
  const factory CashManagementEvent.closeShift(double actualBalance) = _CloseShift;
  const factory CashManagementEvent.toggleBalanceVisibility() = _ToggleBalanceVisibility;
}
...
class CashManagementBloc extends Bloc<CashManagementEvent, CashManagementState> {
  ...
  final ICashDrawerRepository _repository; // Pragmatic choice for "GetCurrent" and "Events"
  ...
  CashManagementBloc(
    ...
    on<_ToggleBalanceVisibility>((_, emit) => emit(state.copyWith(isBalanceVisible: !state.isBalanceVisible)));
  }

  Future<void> _onStarted(_Started event, Emitter<CashManagementState> emit) async {
    ...
    try {
      ...
      if (drawer != null) {
        ...
        emit(state.copyWith(isLoading: false, currentDrawer: drawer, summary: summary));
      } else {
  ...
  Future<void> _onOpenDrawer(_OpenDrawer event, Emitter<CashManagementState> emit) async {
  ...
  Future<void> _onCashDrop(_CashDrop event, Emitter<CashManagementState> emit) async {
     if (state.currentDrawer == null) return;
     emit(state.copyWith(isLoading: true));
     try {
       await _cashDrop(drawerUuid: state.currentDrawer!.id, amount: event.amount, reason: event.reason);
       // Refresh Summary
       final summary = await _getReport(state.currentDrawer!.id);
       final drawer = await _repository.getCurrentDrawer(state.shiftUuid); // Refresh drawer balances
       emit(state.copyWith(isLoading: false, summary: summary, currentDrawer: drawer));
     } catch (e) {
       emit(state.copyWith(isLoading: false, errorMessage: e.toString()));
     }
  }

  Future<void> _onCloseShift(_CloseShift event, Emitter<CashManagementState> emit) async {
    if (state.currentDrawer == null) return;
    emit(state.copyWith(isLoading: true));
    try {
      await _closeShift(drawerUuid: state.currentDrawer!.id, actualClosingBalance: event.actualBalance);
