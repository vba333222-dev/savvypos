          ...
          IconButton(
            ...
            onPressed: () async {
              await Navigator.of(context).push(MaterialPageRoute(
                  builder: (_) => const PurchaseOrderEditorPage()));
              _refresh();
            },
          )
        ],
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Text(
                '${DateFormat.yMMMd().format(po.createdAt!)} • \$${po.totalCost.toStringAsFixed(2)}'),
            trailing: Chip(
              label: Text(status.name.toUpperCase(),
                  style: const TextStyle(fontSize: 10)),
              backgroundColor:
                  _getColorRaw(status).withAlpha((255 * 0.2).round()),
            ),
            onTap: () async {
              await Navigator.of(context).push(MaterialPageRoute(
                  builder: (_) => PurchaseOrderEditorPage(poUuid: po.uuid)));
              _refresh();
            },
          ),
...
class PurchaseOrderEditorPage extends StatefulWidget {
  ...
  @override
  State<PurchaseOrderEditorPage> createState() =>
      _PurchaseOrderEditorPageState();
}
...
class _PurchaseOrderEditorPageState extends State<PurchaseOrderEditorPage> {
  ...
  Future<void> _loadData() async {
    ...
    if (widget.poUuid != null) {
      ...
      if (po != null) {
        _selectedSupplier =
            _suppliers.where((s) => s.uuid == po.supplierUuid).firstOrNull;
  ...
  Future<void> _save(PoStatus newStatus) async {
    ...
    if (_items.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Add Items')));
      return;
    }
    ...
    final po = PurchaseOrder(
      ...
      totalCost: total,
      createdAt: DateTime
          .now(), // Should preserve distinct created at if edit? Repo handles it.
      items: _items
  ...
  Widget build(BuildContext context) {
    ...
    final isReadOnly =
        _status == PoStatus.completed || _status == PoStatus.cancelled;
          ...
          Expanded(
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  leading: Icon(
                      item.isIngredient ? Icons.grass : Icons.inventory_2,
                      color: Colors.indigo),
                  title: Text(item.name),
                  subtitle: Text(
                      '${item.quantity} x \$${item.cost} = \$${(item.quantity * item.cost).toStringAsFixed(2)}'),
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      if (item.received > 0)
                        Chip(
                            label: Text('Rcv: ${item.received}',
                                style: const TextStyle(fontSize: 10)),
...
class _AddItemDialog extends StatefulWidget {
  ...
  const _AddItemDialog(
      {required this.products, required this.ingredients, required this.onAdd});
          ...
          Row(
            children: [
              ...
              ToggleButtons(
                ...
                children: const [
                  Padding(padding: EdgeInsets.all(8.0), child: Text('Product')),
                  Padding(
                      padding: EdgeInsets.all(8.0), child: Text('Ingredient'))
                ],
              ),
            ],
          ),
          ...
          if (!_isIngredient)
            DropdownButtonFormField<Product>(
              ...
              onChanged: (v) => setState(() {
                _selectedProduct = v;
                _costCtrl.text = v?.price.toString() ??
                    '0.0'; // Cost probably different from price but useful default
              }),
            )
          else
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                        flex: 1,
                        child:
                            Text('Ordered: ${item.quantity - item.received}')),
                    const Gap(8),
                    SizedBox(
                        width: 80,
                        child: TextFormField(
