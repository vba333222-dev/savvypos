...
class KitchenTicketTable extends Table {
  ...
  // Status
  TextColumn get status => text().withDefault(const Constant('fired'))(); // fired, prep, ready, served, voided
...
class KitchenItemTable extends Table {
  IntColumn get id => integer().autoIncrement()();
  TextColumn get uuid => text().unique()();
  TextColumn get ticketUuid => text().references(KitchenTicketTable, #uuid, onDelete: KeyAction.cascade)();
  ...
  // KDS Specifics
  TextColumn get course => text().withDefault(const Constant('main'))(); // course enum
