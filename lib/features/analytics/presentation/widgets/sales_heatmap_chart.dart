...
class _SalesHeatmapChartState extends State<SalesHeatmapChart> {
      ...
      child: Column(
        ...
        children: [
            ...
            child: Row(
              children: List.generate(24, (hour) {
                final hourData = widget.data.firstWhere(
                  (d) => d.hour == hour,
                  orElse: () => HourlySalesData(hour: hour, sales: 0, transactions: 0),
                );
                final intensity = _maxSales > 0 ? hourData.sales / _maxSales : 0.0;
                      ...
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          ...
                          colors: [
                            ...
                            _getHeatColor(intensity, colors).withValues(alpha: 0.6),
                          ],
                        ),
                ...
                .animate()
                .scaleY(begin: 0, end: 1, duration: 400.ms, delay: (hour * 20).ms, curve: Curves.easeOutQuad);
              }),
            ),
          ),
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('12AM', style: TextStyle(color: colors.textSecondary, fontSize: 10)),
              Text('6AM', style: TextStyle(color: colors.textSecondary, fontSize: 10)),
              Text('12PM', style: TextStyle(color: colors.textSecondary, fontSize: 10)),
              Text('6PM', style: TextStyle(color: colors.textSecondary, fontSize: 10)),
              Text('11PM', style: TextStyle(color: colors.textSecondary, fontSize: 10)),
            ],
          ),
          ...
          Row(
            ...
            children: [
              _buildLegendItem('Low', colors.textSecondary.withValues(alpha: 0.3), colors),
  ...
  Widget _buildSelectedHourInfo(SavvyColors colors) {
    final hourData = widget.data.firstWhere(
      ...
      orElse: () => HourlySalesData(hour: _selectedHour!, sales: 0, transactions: 0),
    );
    
    final hourStr = _selectedHour! == 0 ? '12AM' 
        : _selectedHour! < 12 ? '${_selectedHour!}AM' 
        : _selectedHour! == 12 ? '12PM' 
        : '${_selectedHour! - 12}PM';
  ...
  Color _getHeatColor(double intensity, SavvyColors colors) {
    if (intensity < 0.3) return colors.textSecondary.withValues(alpha: 0.2 + intensity);
    if (intensity < 0.6) return colors.stateWarning.withValues(alpha: 0.5 + intensity * 0.5);
